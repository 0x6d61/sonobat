...
| SQLite | better-sqlite3 | 同期API。`@types/better-sqlite3` も導入 |
| MCP | @modelcontextprotocol/sdk | MCP TypeScript SDK（stdio トランスポート） |
| XML パーサー | fast-xml-parser | nmap XML パース用。Pure JS |
| ビルド | tsup | esbuild ベース。高速バンドル |
| リンター | ESLint + @typescript-eslint | `eslint.config.mjs`（flat config） |
...
│   ├── mcp/                  # MCP Server（LLM Agent インターフェース）
│   ├── cli/                  # CLI コマンド定義
│   └── types/                # 共有型定義
├── tests/                    # テストファイル（src と同じ構造をミラー）
│   ├── db/
│   ├── parser/
│   ├── engine/
│   └── mcp/
