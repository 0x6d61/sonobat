...
| ツール名 | 引数 | 説明 |
|---------|------|------|
| `list_facts` | `predicate?`, `limit?` | DB の現状を Datalog ファクトとして表示 |
| `run_datalog` | `program`, `save_name?`, `generated_by?` | カスタム Datalog プログラムを実行。ルール保存可 |
| `query_attack_paths` | `pattern` | プリセットまたは保存済みパターンで攻撃パス分析 |
...
- **Datalog セミナイーブ評価** — 現在のナイーブ評価器をセミナイーブ評価に置き換え（大規模ファクト対応）
...
Inspector UI からツール一覧（17件）の確認、`add_host` / `list_hosts` / `propose` 等の動作確認が可能。
